...
/**
 * Handles the heavy lifting for mine resets using the FastAsyncWorldEdit (FAWE) API.
 * Supports layered cinematic animations, schematic pasting, and percentage-based randomization.
 */
public class FAWEResetEngine {

    /** Helper class to handle weighted randomized block selection efficiently. */
    private static class BlockPool {
        ...
        /** Evaluates out of 100% threshold to select a block or return AIR. */
    ...
    /**
     * Executes the reset using FAWE EditSessions. Runs asynchronously if animation speed dictates.
     */
    public static void resetMineLayered(PrisonMinesX plugin, Mine mine) {
        ...
        // Route to schematic logic if defined
        ...
        // High-Performance Instant Reset path
        if ("INSTANT".equalsIgnoreCase(style)) {
            try (EditSession editSession = WorldEdit.getInstance().newEditSessionBuilder().world(weWorld).fastMode(true).build()) {
                ...
                // If fill mode is true, set a mask to only overwrite AIR blocks, preserving player-mined structures
        ...
        Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
            ...
            // Pre-calculate animation frames mathematically
            for (int x = 0; x <= w; x++) {
                for (int y = 0; y <= h; y++) {
                    for (int z = 0; z <= d; z++) {
            ...
            try (EditSession editSession = WorldEdit.getInstance().newEditSessionBuilder()
                    .world(weWorld)
                    .fastMode(!isEmpty) // Use fast mode for massive writes unless we need physics updates
                    .build()) {

                if (mine.isFillMode()) editSession.setMask(new BlockTypeMask(editSession.getExtent(), BlockTypes.AIR));

                // Execute frames sequentially with delay
                for (int currentTick = 0; currentTick < totalFrames; currentTick++) {
                    ...
                    try { Thread.sleep(animDelayMs); } catch (InterruptedException ignored) {}
                }
    ...
    /** Helper to calculate geometric animation frames dynamically based on the style. */
    private static int getAnimationFrame(int x, int y, int z, int w, int h, int d, String style, int totalFrames) {
        ...
        switch (style.toUpperCase()) {
            ...
            case "WAVE": value = ((x / safeW) + Math.sin((z / safeD) * Math.PI * 2) * 0.2 + 0.2) / 1.4; break;
            case "DIAMOND": value = (Math.abs(x - cx) + Math.abs(z - cz)) / (cx + cz); break;
            case "SPIRAL":
                double swirl = ((Math.atan2(z - cz, x - cx) + Math.PI) / (2 * Math.PI)) * 2.0 + (Math.sqrt((x - cx) * (x - cx) + (z - cz) * (z - cz)) / Math.sqrt(cx * cx + cz * cz)) * 3.0;
                value = swirl % 1.0; if (value < 0) value += 1.0; break;
            case "TOP_DOWN": value = 1.0 - (y / safeH); break;
            case "BOTTOM_UP": default: value = y / safeH; break;
        }

        return Math.max(0, Math.min(totalFrames - 1, (int) Math.round(value * (totalFrames - 1))));
    }
        ...
        Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
            ...
            try (ClipboardReader reader = format.getReader(new FileInputStream(schemFile))) {
                ...
                try (EditSession editSession = WorldEdit.getInstance().newEditSessionBuilder().world(weWorld).build()) {
                    ...
                    // 4. Find all Placeholder blocks in the CLIPBOARD and overwrite them in the edit session!
